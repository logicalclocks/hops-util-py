...
class Reservations:
    ...
    def mpirun_is_finished(self):
        """Called when mpirun is finished running"""
        with self.lock:
            return self.finished

    def set_finished(self):
        """Set mpirun status as being finished"""
        with self.lock:
            self.finished = True
...
class Server(MessageSocket):
    ...
    def _handle_message(self, sock, msg):
        ...
        elif msg_type == 'REG_MPIRUN_FINISHED':
            self.reservations.set_finished()
            MessageSocket.send(self, sock, 'OK')
        elif msg_type == 'QUERY':
            MessageSocket.send(self, sock, self.reservations.done())
        elif msg_type == 'MPIRUN_FINISHED_QUERY':
            MessageSocket.send(self, sock, self.reservations.mpirun_is_finished())
        elif msg_type == 'QINFO':
...
class Client(MessageSocket):
    ...
    def register_mpirun_finished(self):
        """Register gpu_presence with server"""
        resp = self._request('REG_MPIRUN_FINISHED')
        return resp

    def await_mpirun_finished(self):
        """Poll until all checks have been done to find GPUs are completed, then return True if any GPUs are present"""
        done = False
        while not done:
            done = self._request('MPIRUN_FINISHED_QUERY')
            time.sleep(1)
        return done
